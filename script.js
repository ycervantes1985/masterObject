const pokemon = Object.freeze([
    ...
    const fires = pokemon.filter( p => {return p.types.includes('fire')});


    const tFires = pokemon.filter( p =>p.types.length > 1).map(p=>p);


    const oNames = pokemon.map( p =>p.name);

    const idMayor = pokemon.filter(p=>p.id>99).map( p =>p.name);
